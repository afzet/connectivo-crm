	...
	switch ($value['ActionLog']['action']) {
		...
		case 'view':
			if ($value['ActionLog']['controller'] == 'settings'):
				$link = $html->link($html->image('icons/view.png'), array('controller' => $value['ActionLog']['controller'], 'action'=>'index'), null, null, false);
			else:
				$link = $html->link($html->image('icons/view.png'), array('controller' => $value['ActionLog']['controller'], 'action'=>'view', $value['ActionLog']['params']), null, null, false);
			endif;
			break;
		default:
			$link = '';
